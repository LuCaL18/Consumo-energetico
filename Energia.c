...
void assegnazionevalore(double energia_disponibile,double totale,double*array_dei_valori,int max,double* corrente_per_elemento)
{
    ...
    for(int i=0;i<max;i++)
    {
        array_dei_valori[i]=(corrente_per_elemento[i]*rapporto);//mettiamo un ulteriore limiti per evitare errori in fase di calcolo per la presenza di valori in float
    }
}
